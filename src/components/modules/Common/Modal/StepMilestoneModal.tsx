...
const StepMileStoneItem = React.memo(function MileStoneItemMemo({
  ...
  return (
    <span css={stepItemStyle(step.enable)}>
      <Tooltip placement="top" title={renderTitle}>
        {/* {`${step.stepName} (${convertStepType(step.stepType)})${isLast ? '' : ' → '} `} */}
        <span className="step-name">{`${step.stepName} (${convertStepType(step.stepType)})`}</span>
      </Tooltip>
      <span>{`${isLast ? '' : ' → '}`}</span>
    </span>
  );
...
const stepItemStyle = (enable: boolean) => css`
